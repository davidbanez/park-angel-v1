...
export const Dashboard: React.FC = () => {
  ...
  const [parkingUtilization, setParkingUtilization] = useState<ParkingUtilization[]>([]);
            ...
            {systemHealth && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-sm font-medium ${getStatusColor(systemHealth.apiStatus)}`}>
                    {systemHealth.apiStatus}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Database</span>
                  <span className={`text-sm font-medium ${getStatusColor(systemHealth.databaseStatus)}`}>
                    {systemHealth.databaseStatus}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Payment Gateway</span>
                  <span className={`text-sm font-medium ${getStatusColor(systemHealth.paymentGatewayStatus)}`}>
                    {systemHealth.paymentGatewayStatus}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-600">Real-time</span>
                  <span className={`text-sm font-medium ${getStatusColor(systemHealth.realtimeStatus)}`}>
          ...
          <CardContent>
            <div className="space-y-4">
              {performanceMetrics.slice(0, 6).map((metric) => (
                <div key={metric.feature} className="flex items-center justify-between">
                  <div>
                    <span className="text-sm text-gray-600">{metric.feature}</span>
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="w-16 bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${Math.min(location.occupancyRate, 100)}%` }}
                          />
                        </div>
                      </div>
                    </td>
