...
import { AuthService, SessionService, AuthorizationService, } from '../services/auth';
...
export function useAuth() {
    ...
    useEffect(() => {
        ...
        const initializeAuth = async () => {
            try {
                ...
                if (session && mounted) {
                    const { user, profile, userType } = await AuthService.getCurrentUserWithProfile();
                    setState({
                        user,
                        session,
                        userType,
    ...
    const refreshSession = useCallback(async () => {
        if (!state.session)
            return;
        try {
            const refreshedSession = await SessionService.refreshSession(state.session.access_token);
            if (refreshedSession) {
                setState(prev => ({
                    ...prev,
                    session: {
    ...
    const hasPermission = useCallback(async (resource, action, context) => {
        if (!state.user)
            return false;
        try {
            const authContext = await AuthorizationService.createAuthorizationContext(state.user.id);
