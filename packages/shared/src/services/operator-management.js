import { validateQueryResult, safeAccess } from '../lib/supabase';
export class OperatorManagementServiceImpl {
    ...
    async updateOperatorProfile(operatorId, data) {
        try {
            const updateData = {};
            if (data.companyName)
                updateData.company_name = data.companyName;
            if (data.businessRegistrationNumber)
                updateData.business_registration_number = data.businessRegistrationNumber;
            if (data.taxIdentificationNumber)
                updateData.tax_identification_number = data.taxIdentificationNumber;
            ...
            if (data.licenseNumber)
                updateData.license_number = data.licenseNumber;
            if (data.licenseExpiryDate)
                updateData.license_expiry_date = data.licenseExpiryDate;
            if (data.verificationDocuments)
                updateData.verification_documents = data.verificationDocuments;
            const { data: profile, error } = await this.supabase
                .from('operator_profiles')
                .update(updateData)
                .eq('operator_id', operatorId)
    ...
    async verifyOperator(operatorId) {
        try {
    ...
    async getBankDetails(operatorId) {
        try {
            const { data, error } = await this.supabase
                .from('operator_bank_details')
                .select('*')
    ...
    async updateBankDetails(bankDetailId, data) {
        try {
            const updateData = {};
            if (data.bankName)
            ...
            if (data.routingNumber)
                updateData.routing_number = data.routingNumber;
            if (data.swiftCode)
                updateData.swift_code = data.swiftCode;
            if (data.branchName)
                updateData.branch_name = data.branchName;
            if (data.branchAddress)
                updateData.branch_address = data.branchAddress;
            if (data.accountType)
    ...
    async setPrimaryBankAccount(bankDetailId) {
        try {
            const { error } = await this.supabase
                .from('operator_bank_details')
    ...
    async verifyBankAccount(bankDetailId) {
        try {
            const { error } = await this.supabase
                .from('operator_bank_details')
                .update({ is_verified: true })
    ...
    async getRevenueConfigs(operatorId) {
        try {
            const { data, error } = await this.supabase
                .from('operator_revenue_configs')
                .select('*')
    ...
    async updateRevenueConfig(configId, operatorPercentage, parkAngelPercentage) {
        try {
            const { data: config, error } = await this.supabase
                .from('operator_revenue_configs')
                .update({
    ...
    async createVIPAssignment(data) {
        try {
    ...
    async getVIPAssignments(operatorId) {
        try {
            const { data, error } = await this.supabase
                .from('vip_assignments')
                .select(`
    ...
    async updateVIPAssignment(assignmentId, data) {
        try {
            const updateData = {};
            if (data.vipType)
                updateData.vip_type = data.vipType;
            if (data.locationId)
                updateData.location_id = data.locationId;
            if (data.spotIds)
                updateData.spot_ids = data.spotIds;
            if (data.timeLimitMinutes)
                updateData.time_limit_minutes = data.timeLimitMinutes;
            if (data.validFrom)
                updateData.valid_from = data.validFrom;
            if (data.validUntil)
                updateData.valid_until = data.validUntil;
            if (data.notes)
    ...
    async deactivateVIPAssignment(assignmentId) {
        try {
            const { error } = await this.supabase
                .from('vip_assignments')
    ...
    async calculateRemittance(operatorId, periodStart, periodEnd) {
        try {
            ...
            const totalRevenue = revenues.reduce((sum, share) => {
                const grossAmount = typeof share.gross_amount === 'number' ? share.gross_amount : 0;
                return sum + grossAmount;
            }, 0);
            const operatorShare = revenues.reduce((sum, share) => {
                const opShare = typeof share.operator_share === 'number' ? share.operator_share : 0;
                return sum + opShare;
            }, 0);
            const parkAngelShare = revenues.reduce((sum, share) => {
                const platformFee = typeof share.platform_fee === 'number' ? share.platform_fee : 0;
    ...
    async createRemittance(operatorId, calculation) {
        try {
    ...
    async getRemittances(operatorId) {
        try {
            const { data, error } = await this.supabase
                .from('operator_remittances')
                .select('*')
    ...
    async processRemittance(remittanceId, processedBy, paymentReference) {
        try {
            const { error } = await this.supabase
                .from('operator_remittances')
                .update({
    ...
    async getAllOperators() {
        try {
            ...
            const operators = await Promise.all((userData || []).map(async (user) => {
                ...
                const metrics = await this.getOperatorDashboardMetrics(userId);
                return {
                    id: userId,
                    companyName: safeAccess(profile, 'company_name', 'Unknown'),
                    contactPerson: safeAccess(profile, 'contact_person', 'Unknown'),
                    contactEmail: safeAccess(profile, 'contact_email', String(user.email)),
    ...
    async getOperatorDashboardMetrics(operatorId) {
        try {
            ...
            const totalLocations = locationData?.length || 0;
            ...
            if (locationData && locationData.length > 0) {
                ...
                const { data: sectionData } = await this.supabase
                    .from('sections')
                    .select('id')
                    .in('location_id', locationData.map(l => String(l.id)));
                if (sectionData && sectionData.length > 0) {
                    // Get zones for these sections
                    const { data: zoneData } = await this.supabase
                        .from('zones')
                        .select('id')
                        .in('section_id', sectionData.map(s => String(s.id)));
                    if (zoneData && zoneData.length > 0) {
                        // Get spots for these zones
                        const { data: spotData } = await this.supabase
                            .from('parking_spots')
                            .select('id, status')
                            .in('zone_id', zoneData.map(z => String(z.id)));
                        if (spotData) {
                            totalSpots = spotData.length;
                            occupiedSpots = spotData.filter(spot => spot.status === 'occupied').length;
                        }
                    }
                }
            }
            ...
            const todayRevenue = revenues
                ...
                .reduce((sum, r) => {
                const operatorShare = typeof r.operator_share === 'number' ? r.operator_share : 0;
                return sum + operatorShare;
            }, 0);
            ...
            const lastRemittanceDate = lastRemittanceData?.[0]?.processed_at
                ? new Date(String(lastRemittanceData[0].processed_at))
                : undefined;
            return {
                operatorId,
                totalLocations,
    ...
    async getOperatorPerformanceMetrics(operatorId, startDate, endDate) {
        try {
