...
export class AdvertisementManagementService {
  ...
  async listAdvertisements(
    ...
    const advertisements = data?.map((item: any) => this.mapDatabaseToAdvertisement(item)) || [];
  ...
  async updateAdvertisement(id: string, request: UpdateAdvertisementRequest): Promise<Advertisement> {
    ...
    if (request.targetLocationId || request.targetType || request.startDate || request.endDate) {
      ...
      const conflicts = await this.checkConflicts(
        request.targetLocationId || current.targetLocationId,
        request.targetType || current.targetType,
    ...
    if (request.contentType !== undefined) updateData.content_type = request.contentType;
    if (request.contentUrl !== undefined) updateData.content_url = request.contentUrl;
    if (request.contentText !== undefined) updateData.content_text = request.contentText;
    if (request.targetLocationId !== undefined) updateData.target_location_id = request.targetLocationId;
    if (request.targetType !== undefined) updateData.target_type = request.targetType;
    if (request.startDate !== undefined) updateData.start_date = request.startDate.toISOString();
    if (request.endDate !== undefined) updateData.end_date = request.endDate.toISOString();
    if (request.budget !== undefined) updateData.budget = request.budget;
    if (request.costPerImpression !== undefined) updateData.cost_per_impression = request.costPerImpression;
    if (request.costPerClick !== undefined) updateData.cost_per_click = request.costPerClick;
   ...
   */
  async getAdvertisementMetrics(advertisementId: string, dateRange?: { startDate: Date; endDate: Date }): Promise<AdMetrics[]> {
    let query = supabase
   ...
   */
  async getPerformanceReport(advertisementId: string, dateRange?: { startDate: Date; endDate: Date }): Promise<AdPerformanceReport> {
  ...
  async getAnalytics(): Promise<AdAnalytics> {
    ...
    const activeAds = counts?.filter((ad: any) => ad.status === 'active').length || 0;
    const pendingApproval = counts?.filter((ad: any) => ad.status === 'pending').length || 0;
    ...
    const totalImpressions = metricsData?.reduce((sum: number, m: any) => sum + m.impressions, 0) || 0;
    const totalClicks = metricsData?.reduce((sum: number, m: any) => sum + m.clicks, 0) || 0;
    const totalRevenue = metricsData?.reduce((sum: number, m: any) => sum + parseFloat(m.total_cost), 0) || 0;
    const averageCTR = totalImpressions > 0 ? (totalClicks / totalImpressions) * 100 : 0;
    ...
    const topPerformingAds = topAds?.map((ad: any) => ({
      id: ad.advertisement_id,
      title: (ad.advertisements as any).title,
  ...
  // Helper methods
  private mapDatabaseToAdvertisement(data: any): Advertisement {
    return {
      id: data.id,
      title: data.title,
