-- Operator Management System Migration
-- This migration adds comprehensive operator management functionality
...
-- Add unique constraint for primary bank account per operator
CREATE UNIQUE INDEX IF NOT EXISTS unique_primary_bank_per_operator 
ON operator_bank_details(operator_id) 
WHERE is_primary = TRUE;

-- Revenue sharing configurations per operator
CREATE TABLE IF NOT EXISTS operator_revenue_configs (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  operator_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS operator_remittances (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  operator_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_operator_profiles_operator_id ON operator_profiles(operator_id);
CREATE INDEX IF NOT EXISTS idx_operator_profiles_is_verified ON operator_profiles(is_verified);
CREATE INDEX IF NOT EXISTS idx_operator_bank_details_operator_id ON operator_bank_details(operator_id);
CREATE INDEX IF NOT EXISTS idx_operator_bank_details_is_primary ON operator_bank_details(is_primary);
CREATE INDEX IF NOT EXISTS idx_operator_revenue_configs_operator_id ON operator_revenue_configs(operator_id);
CREATE INDEX IF NOT EXISTS idx_operator_revenue_configs_parking_type ON operator_revenue_configs(parking_type);
CREATE INDEX IF NOT EXISTS idx_operator_revenue_configs_effective_date ON operator_revenue_configs(effective_date);
CREATE INDEX IF NOT EXISTS idx_operator_remittances_operator_id ON operator_remittances(operator_id);
CREATE INDEX IF NOT EXISTS idx_operator_remittances_status ON operator_remittances(status);
CREATE INDEX IF NOT EXISTS idx_operator_remittances_period ON operator_remittances(period_start, period_end);
...
CREATE INDEX IF NOT EXISTS idx_vip_assignments_vip_type ON vip_assignments(vip_type);
CREATE INDEX IF NOT EXISTS idx_vip_assignments_is_active ON vip_assignments(is_active);
CREATE INDEX IF NOT EXISTS idx_operator_performance_metrics_operator_id ON operator_performance_metrics(operator_id);
CREATE INDEX IF NOT EXISTS idx_operator_performance_metrics_date ON operator_performance_metrics(metric_date);
...
ALTER TABLE operator_revenue_configs ENABLE ROW LEVEL SECURITY;
ALTER TABLE operator_remittances ENABLE ROW LEVEL SECURITY;
ALTER TABLE vip_assignments ENABLE ROW LEVEL SECURITY;
ALTER TABLE operator_performance_metrics ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for operator profiles
CREATE POLICY "Operators can view their own profile" ON operator_profiles
  FOR SELECT USING (operator_id = auth.uid());

CREATE POLICY "Operators can update their own profile" ON operator_profiles
  FOR UPDATE USING (operator_id = auth.uid());

CREATE POLICY "Admins can view all operator profiles" ON operator_profiles
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() 
...
-- Create RLS policies for operator bank details
CREATE POLICY "Operators can manage their own bank details" ON operator_bank_details
  FOR ALL USING (operator_id = auth.uid());

CREATE POLICY "Admins can view all bank details" ON operator_bank_details
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() 
...
-- Create RLS policies for revenue configs
CREATE POLICY "Operators can view their own revenue configs" ON operator_revenue_configs
  FOR SELECT USING (operator_id = auth.uid());

CREATE POLICY "Admins can manage all revenue configs" ON operator_revenue_configs
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() 
...
-- Create RLS policies for remittances
CREATE POLICY "Operators can view their own remittances" ON operator_remittances
  FOR SELECT USING (operator_id = auth.uid());

CREATE POLICY "Admins can manage all remittances" ON operator_remittances
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() 
...
-- Create RLS policies for VIP assignments
CREATE POLICY "Operators can manage VIP assignments for their users" ON vip_assignments
  FOR ALL USING (operator_id = auth.uid());

CREATE POLICY "Users can view their own VIP assignments" ON vip_assignments
  FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "Admins can manage all VIP assignments" ON vip_assignments
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() 
...
CREATE POLICY "Operators can view their own performance metrics" ON operator_performance_metrics
  FOR SELECT USING (operator_id = auth.uid());

CREATE POLICY "Admins can view all performance metrics" ON operator_performance_metrics
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM users 
      WHERE id = auth.uid() 
...
-- Insert default revenue sharing configurations (only if there are operators and admins)
INSERT INTO operator_revenue_configs (operator_id, parking_type, operator_percentage, park_angel_percentage, created_by)
SELECT 
  u.id,
  'street'::parking_type,
  70.00,
  30.00,
  COALESCE((SELECT id FROM users WHERE user_type = 'admin' LIMIT 1), u.id)
FROM users u
WHERE u.user_type = 'operator'
AND EXISTS (SELECT 1 FROM users WHERE user_type = 'admin')
AND NOT EXISTS (
  SELECT 1 FROM operator_revenue_configs orc 
  WHERE orc.operator_id = u.id 
  AND orc.parking_type = 'street'
);
...
AND NOT EXISTS (
  SELECT 1 FROM operator_revenue_configs orc 
  WHERE orc.operator_id = u.id 
  AND orc.parking_type = 'hosted'
);

-- Create function to automatically create revenue configs for new operators
CREATE OR REPLACE FUNCTION create_default_operator_configs()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.user_type = 'operator' THEN
    -- Create default revenue sharing configurations
    INSERT INTO operator_revenue_configs (operator_id, parking_type, operator_percentage, park_angel_percentage, created_by)
    VALUES 
      (NEW.id, 'street', 70.00, 30.00, NEW.id),
      (NEW.id, 'facility', 75.00, 25.00, NEW.id),
      (NEW.id, 'hosted', 0.00, 40.00, NEW.id);
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER create_default_operator_configs_trigger
  AFTER INSERT ON users
  FOR EACH ROW EXECUTE FUNCTION create_default_operator_configs();